...
Only because we knew that there were a benefit of note taking in longhand compared to laptop format in the experimental task developed by @muellerPenMightierKeyboard2014, this does not mean that such a benefit always occurs. For example, the experiment only used a rather short video lectures of around 15 minutes in length. Would such a benefit also occur for longer lectures? Likewise, does this effect only appear for the relatively well educated participants of the original study or really apply to the whole population of students? Another potential issue is how long the memory benefit holds. In the experiments considered here, the memory task followed around thirty minutes after the lecture. @muellerPenMightierKeyboard2014 addressed this question in their Experiment 3, where testing took place roughly one week after the virtual and still found a longhand memory benefit. However, this result has as of yet not been replicated. 

What this list of open questions shows is that even in the best case scenario in which we knew that the failed replication were not a problem, the implications from the statistical result for our research question would still be limited. As discussed in detail in Section \@ref{#epistemic-gap-1}, the largest problem for a strong inference from any statistical result is the underdetermination of theory by data. Only because we find a specific pattern in one task, this does pattern generalises to all instances for which we hope our research question applies. The most important -- and perhaps most difficult -- task for us as researchers, is to not fall into the trap and interpret a significant $p$-value such that it means that our research question is answered. It only is one piece of evidence in the research process.

In sum, the results suggests that if there is a benefit of a particular type of note taking on conceptual memory, it is not easy to detect. In any case, the available data does not suggest that everyone should ditch their laptop for paper and pencil. If researchers feel that this is an important research question, they need to provide stronger evidence. 

## Summary

In this chapter, we have discussed three things. Firstly, we have used the `tidyverse` and prepared the data from two Experiments of @muellerPenMightierKeyboard2014 for analysis. We have seen that such a data preparation often not only requires the necessary `tidyverse` skills, but also knowledge of the data set. As part of this data preparation, we have discussed the importance of performing coherence checks to ensure the data matches our assumptions. Only when we have assured ourselves that the data is okay should we go ahead with the statistical analysis.

Secondly, we have used `afex::aov_car()` and re-analysed three data sets using NHST. These three data sets shared the same structure, one continuous dependent variable and one categorical independent variable with two levels, and we could therefore use ANOVA for the analysis. As part of this analysis we have also created results figures using `afex_plot()` and performed follow-up analyses using `emmeans()`. As part of this analysis we have seen that once the data set is in the correct format, estimating the statistical model and obtaining the $p$-value is relatively easy and straight forward. 

Thirdly, we have discussed what we learn from the three different experiments for the research question on how different types of note taking during lectures affect the conceptual memory for the lecture content. We have seen that despite the results from @muellerPenMightierKeyboard2014 that suggests that note taking in longhand format provides a memory benefit compared to note taking with a laptop, the conclusion is not that clear. One problem is that the replication of @urry2021 questions whether the original pattern is a genuine effect that holds in the population. The other problem is that even if the results of @muellerPenMightierKeyboard2014 would reliable replicate, that would not provide a conclusive answer to the general research question. Only because we find something in one particular task, this does not mean it holds for the underlying research question. In the reality of research, the epistemic problems discussed in Chapter \@ref{role-of-statistics-in-the-research-process} cannot be ignored.

The overarching goal of this chapter was to paint a realistic picture of what we can learn from a significant result. One reality of research practice is that a significant results is generally what researchers are looking for. If a results is significant we are happy -- our experiment "has worked" and we can publish it. If the result is not significant we are unhappy, because we generally have problems publishing our results. Whereas this might be the reality of research practice, the significant $p$-value is not actually the end goal. Even if we find a significant $p$-value what we can learn from a particular study is always limited. And if we use an established task or study an established phenomenon and find a non-significant result, that can also be of interest. In many domains the published literature paints a picture that it is easy to obtain a particular result, but the reality often is that it is not because researcher do not publish the failed attempts. In sum, please do not confuse the $p$-value with the answer to your research question.
