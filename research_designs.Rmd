...
Let us discuss the last point from above in more detail. For many people, when they think about data, they think about numbers. As we have seen here this is not necessary, data does not have to be numbers. The example of the `response` variable shows that we can use other values, such as words or phrases, to represent values of variables. However, the conception of data primarily as numbers is also not false. For example, the statistical analyses introduced in this book need to represent all variables in terms of numbers. Fortunately for us, the software we will be using will generally help us and convert non-numeric data into numeric data when necessary. This means we can use the type of representation that is the most intuitive and easiest to understand.

So far we have only distinguished numeric data -- data represented through numbers -- and non-numeric data. As we will see in the following, this distinction is not enough and we might need to define further *data types*. Let us take a more in depth look at the four numeric variables we have in the data, `subno`, `loss`, `gain`, and `resp`. As we have seen above, `loss` and `gain` contain the same type of information, a potential loss or gain, so we can consider them together as one variable pair `loss`/`gain`, which leaves us with three variables. If we compare these three variables, we see that even though they are all using numbers, the meaning of numbers differs considerable.

Let us begin with the easiest case, the `loss`/`gain` pair. Here, the meaning of numbers corresponds to the usual understanding of numbers, as a magnitude of something, particularly the magnitude of a potential gain and loss. We could understand these variables also as *measuring* the potential gain and loss of a gamble. Because the values of the variables represent numeric information in this narrow sense (i.e., as actual numbers of something) that allows us to treat them as numbers in a mathematical sense and perform mathematical operations on them. For example, we could calculate the average potential loss/gain for a participant in a condition.

For `subno` the numbers do not really measure the magnitude of something. Participant number 16 is not twice participant 8. As introduced here, the only inherent meaning of the number is that a participant with a smaller number participant in the experiment earlier than a participant with a larger number. However, in the present case, the order in which participants participated in the experiment does not matter, we could also reshuffle the numbers across participants (i.e., assign each participant a random number of the ones used). We could also imagine an experiment in which participant numbers are assigned to participants in a random order. What this means is that the number here only serves the purpose of a *label*. We need to identify which participant a specific observation refers to, but this is the only reason we use numbers here. We could also use non-numeric labels here such as random letters. Consequently, it would not make much sense to perform any mathematical operation on the `subno` variable. For example, the average participant number does not provide any useful information.

Finally,

If we compare `loss`/`gain` with `subno` we can the difference between both is in the meaning of the numbers. For `loss`/`gain` the numbers correspond to a measure of the magnitude of the gain and loss. For `subno` the numbers corresponds to a label and does not carry any inherent numerical information other than the order in which participants started the experiment. If we were to ignore the
